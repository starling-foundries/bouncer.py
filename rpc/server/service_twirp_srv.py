...
class OperatorImpl(object):
    """
    ============================================================================
    Primitives
    ============================================================================
    """
    def SendCheck(self, check_transaction, ctx={}):
        """
        sends the signed transaction from the client to the operator.
        """
        raise TwirpServerException(Errors.Unimplemented,
                                   "SendCheck is unimplemented")

    def Status(self, queue_i_d, ctx={}):
        """
    ...
    def GetLastTransaction(self, client, ctx={}):
        """
        rpc RevokeOperator(Empty) returns (StatusResp); 
        Gets the status of the last transaction sent by this user to this operator
        """
        raise TwirpServerException(Errors.Unimplemented,
                                   "GetLastTransaction is unimplemented")

    def OperatorTarget(self, empty, ctx={}):
        raise TwirpServerException(Errors.Unimplemented,
                                   "OperatorTarget is unimplemented")

    def GetMinFee(self, empty, ctx={}):
        raise TwirpServerException(Errors.Unimplemented,
                                   "GetMinFee is unimplemented")


class OperatorServer(TwirpWSGIApp):
    def __init__(self, service):
        """Creates a new WSGI app for the Operator service.

        Args:
            service: An object with methods matching the protocol of
                OperatorImpl, which implements the service logic.
        """
        self.service = service

        self._package_name = "starlingfoundries.operator.server"
        self._service_name = "starlingfoundries.operator.server.Operator"
        self._prefix = "/twirp/" + self._service_name

        self._endpoints = {
            "SendCheck": Endpoint(
                name="SendCheck",
                function=getattr(service, "SendCheck"),
                input=_sym_lookup("starlingfoundries.operator.server.CheckTransaction"),
                output=_sym_lookup("starlingfoundries.operator.server.queueID"),
            ),
            "Status": Endpoint(
                name="Status",
                function=getattr(service, "Status"),
                input=_sym_lookup("starlingfoundries.operator.server.queueID"),
                output=_sym_lookup("starlingfoundries.operator.server.StatusResp"),
            ),
            "GetLastTransaction": Endpoint(
                name="GetLastTransaction",
                function=getattr(service, "GetLastTransaction"),
                input=_sym_lookup("starlingfoundries.operator.server.Client"),
                output=_sym_lookup("starlingfoundries.operator.server.StatusResp"),
            ),
            "OperatorTarget": Endpoint(
                name="OperatorTarget",
                function=getattr(service, "OperatorTarget"),
                input=_sym_lookup("starlingfoundries.operator.server.Empty"),
                output=_sym_lookup("starlingfoundries.operator.server.OperatorTargetResp"),
            ),
            "GetMinFee": Endpoint(
                name="GetMinFee",
                function=getattr(service, "GetMinFee"),
                input=_sym_lookup("starlingfoundries.operator.server.Empty"),
                output=_sym_lookup("starlingfoundries.operator.server.FeeResp"),
            ),
        }
